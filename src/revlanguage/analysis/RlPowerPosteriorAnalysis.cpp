...
const MemberRules& PowerPosteriorAnalysis::getParameterRules(void) const
{
    ...
    if ( rules_set == false )
    {
        ...
        member_rules.push_back( new ArgumentRule("cats"       , Natural::getClassTypeSpec()                 , "Number of power posteriors (beta < 1) to run in addition to the actual posterior (beta = 1) if no powers are specified.", ArgumentRule::BY_VALUE, ArgumentRule::ANY, new Natural(100) ) );
