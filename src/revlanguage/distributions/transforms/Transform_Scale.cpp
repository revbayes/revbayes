...
Transform_Scale* Transform_Scale::clone( void ) const
{
...
const std::string& Transform_Scale::getClassType(void)
{

    static std::string rev_type = "Transform_Scale";
 ...
 */
std::string Transform_Scale::getDistributionFunctionName( void ) const
{
...
const MemberRules& Transform_Scale::getParameterRules(void) const
{
	...
	std::vector<TypeSpec> distTypes = { TypedDistribution<Real>::getClassTypeSpec(), TypedDistribution<RealPos>::getClassTypeSpec(), TypedDistribution<Probability>::getClassTypeSpec()};
        ...
        dist_member_rules.push_back( new ArgumentRule( "lambda", Real::getClassTypeSpec()   , "The factor multiplied by the base random variable.", ArgumentRule::BY_CONSTANT_REFERENCE, ArgumentRule::ANY ) );
...
const TypeSpec& Transform_Scale::getTypeSpec( void ) const
{
...
void Transform_Scale::setConstParameter(const std::string& name, const RevPtr<const RevVariable> &var)
{
