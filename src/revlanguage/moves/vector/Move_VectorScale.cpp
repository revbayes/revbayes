...
void Move_VectorScale::constructInternalObject( void )
{
    ...
    RevBayesCore::StochasticNode<RevBayesCore::RbVector<double> > *n = RevBayesCore::assumeStochastic<RevBayesCore::RbVector<double> >( tmp );
...
const MemberRules& Move_VectorScale::getParameterRules(void) const
{
    ...
    if ( !rules_set )
    {
        ...
        move_member_rules.push_back( new ArgumentRule( "tuneTarget", Probability::getClassTypeSpec(), "The acceptance probability targeted by auto-tuning.", ArgumentRule::BY_VALUE, ArgumentRule::ANY, new Probability( 0.23 ) ) );
