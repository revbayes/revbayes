...
const MemberRules& Move_EllipticalSliceSamplingSimple::getParameterRules(void) const
{
    ...
    if ( !rules_set )
    {
        ...
        /* Inherit weight (but not tuneTarget!) from Move and put it after the arguments created above. Strictly speaking,
         * EllipticalSliceSamplingLognormalIIDMove does have a tuning parameter, but it doesn't make sense to let its value
         * approach a user-set target.
         */
