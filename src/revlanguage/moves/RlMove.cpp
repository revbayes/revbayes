...
const MemberRules& Move::getParameterRules(void) const
{
    ...
    if ( !rules_set )
    {
        move_member_rules.push_back( new ArgumentRule( "weight", RealPos::getClassTypeSpec(), "The weight defines how often on average this move will be used per iteration. The number of moves proposed per iteration is the sum of the weights of all moves.", ArgumentRule::BY_VALUE, ArgumentRule::ANY, new RealPos( 1.0 ) ) );
