...
void Move_MatrixSingleElementScale::constructInternalObject( void )
{
    ...
    if ( rval > 0 ) static_cast<RevBayesCore::MatrixRealSingleElementScaleProposal*>(p)->setRow( size_t(rval) );
    long cval = static_cast<const Natural &>( col->getRevObject() ).getValue();
    if ( cval > 0 ) static_cast<RevBayesCore::MatrixRealSingleElementScaleProposal*>(p)->setColumn( size_t(cval) );
