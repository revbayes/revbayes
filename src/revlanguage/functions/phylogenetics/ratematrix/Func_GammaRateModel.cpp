...
Core::ConcreteMixtureModel* GammaRateModelFunc(const Core::MixtureModel& submodel, double a, int nCats, Core::Boolean median)
{
    using namespace RevBayesCore;
...
Core::TypedFunction< Core::MixtureModel >* Func_GammaRateModel::createFunction( void ) const
{
    Core::TypedDagNode< Core::MixtureModel >* submodel = dynamic_cast<const MixtureModel &>( this->args[0].getVariable()->getRevObject() ).getDagNode();
    ...
    return Core::generic_function_ptr< Core::MixtureModel >( GammaRateModelFunc, submodel, a, nCats, median);
}
...
const ArgumentRules& Func_GammaRateModel::getArgumentRules( void ) const
{
    ...
    if ( !rules_set )
    {
        argumentRules.push_back( new ArgumentRule( "submodel"       , MixtureModel::getClassTypeSpec(), "Sub-model.", ArgumentRule::BY_CONSTANT_REFERENCE, ArgumentRule::ANY ) );
