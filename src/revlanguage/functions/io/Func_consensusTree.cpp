...
RevPtr<RevVariable> Func_consensusTree::execute(void)
{
    ...
    bool differentiate_samp_anc = static_cast<const RlBoolean &>( this->args[arg_index++].getVariable()->getRevObject() ).getValue();
    RevBayesCore::Tree* tree = summary->mrTree(report, cutoff, true, differentiate_samp_anc);
...
const ArgumentRules& Func_consensusTree::getArgumentRules( void ) const
{
    ...
    if (!rules_set)
    {
        ...
        argumentRules.push_back( new ArgumentRule( "differentiateMRCAs" , RlBoolean::getClassTypeSpec() , "Should we treat clades of identical composition as different if they differ in their most recent common ancestor?", ArgumentRule::BY_VALUE, ArgumentRule::ANY, new RlBoolean(false) ) );
