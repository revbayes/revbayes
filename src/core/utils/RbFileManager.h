...
namespace RevBayesCore {
    ...
    void                    truncateMonitorFileAfterGeneration(const path& file_name, std::uint64_t lastGen); //!< Truncate a tabular or JSON monitor-style trace so the first row whose generation exceeds lastGen is removed
}
