...
//        AbstractPhyloCTMCSiteHomogeneous(const TypedDagNode<Tree> *t, size_t nChars, size_t nMix, bool c, size_t nSites, bool amb, bool wd = false, bool internal = false, bool gapmatch = true );
    ...
    protected:
        ...
        // the likelihoods
        double*                                             cladopartial_branch_likelihoods;
...
RevBayesCore::PhyloCTMCClado<charType>::PhyloCTMCClado(const TypedDagNode<Tree> *t, size_t nChars, bool c, size_t nSites, bool amb, bool internal, bool gapmatch) : AbstractPhyloCTMCSiteHomogeneous<charType>(  t, nChars, 1, c, nSites, amb, false, false, true ),
    ...
    cladopartial_branch_likelihoods(NULL),
...
//    const TypedDagNode< RevBayesCore::RbVector<RevBayesCore::RbVector<long> > >* events, const TypedDagNode<RevBayesCore::RbVector<double> >* probs, int n_states 
    ...
    cladoactive_likelihood_offset      =  this->num_nodes*this->num_site_rates*this->num_patterns*this->num_states*this->num_states;
    cladonode_offset                  =                  this->num_site_rates*this->num_patterns*this->num_states*this->num_states;
    cladomixture_offset               =                                       this->num_patterns*this->num_states*this->num_states;
    cladosite_offset                  =                                                          this->num_states*this->num_states;
    ...
    branchHeterogeneousCladogenesis(n.branchHeterogeneousCladogenesis)
{
    ...
    {
        cladopartial_branch_likelihoods = new double[2*this->num_nodes*this->num_site_rates*this->num_sites*this->num_states*this->num_states];
        memcpy(cladopartial_branch_likelihoods, n.cladopartial_branch_likelihoods, 2*this->num_nodes*this->num_site_rates*this->num_patterns*this->num_states*this->num_states*sizeof(double));
    }
    
    // copy the marginal likelihoods if necessary
    if ( this->useMarginalLikelihoods == true )
    {
        cladoMarginalLikelihoods = new double[this->num_nodes*this->num_site_rates*this->num_sites*this->num_states*this->num_states];
        memcpy(cladoMarginalLikelihoods, n.cladoMarginalLikelihoods, this->num_nodes*this->num_site_rates*this->num_sites*this->num_states*this->num_states*sizeof(double));
    }
...
RevBayesCore::PhyloCTMCClado<charType>::~PhyloCTMCClado( void ) {
    ...
    delete [] cladopartial_branch_likelihoods;
...
void RevBayesCore::PhyloCTMCClado<charType>::computeRootLikelihood( size_t root, size_t left, size_t right)
{
    ...
    double* p_node         = this->partial_branch_likelihoods + this->active_branch_likelihood[root]  * this->active_branch_likelihood_offset + root  * this->node_offset;
...
template<class charType>
void RevBayesCore::PhyloCTMCClado<charType>::computeInternalNodeLikelihoodBranchWise(const TopologyNode &node, size_t node_index, size_t left, size_t right, size_t middle)
{
	computeInternalNodeLikelihoodBranchWise(node, node_index, left, right);
}

template<class charType>
void RevBayesCore::PhyloCTMCClado<charType>::computeInternalNodeLikelihoodBranchWise(const TopologyNode &node, size_t node_index, size_t left, size_t right)
{
    ...
    double*         p_clado_node  = this->cladopartial_branch_likelihoods + this->active_branch_likelihood[node_index]*this->cladoactive_likelihood_offset + node_index*this->cladonode_offset;
    ...
    for (size_t mixture = 0; mixture < this->num_site_rates; ++mixture)
    {
        ...
        for (size_t site = 0; site < this->num_patterns ; ++site)
        {
            ...
            for (size_t c0 = 0; c0 < this->num_states; ++c0) {
                double sum_ana = 0.0;
...
void RevBayesCore::PhyloCTMCClado<charType>::computeMarginalNodeLikelihood( size_t node_index, size_t parentnode_index )
{
    ...
    const double*   p_clado_node                    = this->cladopartial_branch_likelihoods + this->active_branch_likelihood[node_index]*this->cladoactive_likelihood_offset + node_index*this->cladonode_offset;
            ...
            for (size_t j=0; j<this->num_states; ++j)
            {
                ...
                *p_site_marginal_j = 0.0;
                ...
                for (size_t k=0; k<this->num_states; ++k)
                {
                    ...
                    const double tp_kj = *p_parent_site_marginal_k * tp_begin[ k * this->num_states + j ];
...
void RevBayesCore::PhyloCTMCClado<charType>::resizeLikelihoodVectors( void )
{
    ...
    size_t n = this->num_nodes*this->num_site_rates*this->num_patterns*this->num_states*this->num_states;
    ...
    if ( this->in_mcmc_mode == true )
    {
        ...
        cladopartial_branch_likelihoods = new double[2*n];
        
        // reinitialize likelihood vectors
        for (size_t i = 0; i < 2*n; i++)
        {
            cladopartial_branch_likelihoods[i] = 0.0;
        }
...
//    std::cout << "SIMULATE\n";
    ...
    for ( size_t i = 0; i < this->num_sites; ++i )
    {
        ...
        charType cl = charType( this->num_states );
        charType cr = charType( this->num_states );
        cl.setToFirstState();
        cr.setToFirstState();
        double u = rng->uniform01();
        std::map<std::vector<unsigned>, double>::iterator it;
...
std::vector< std::vector<double> >* RevBayesCore::PhyloCTMCClado<charType>::sumMarginalLikelihoods( size_t node_index )
{
    ...
    std::vector< std::vector<double> >* per_mixture_Likelihoods = new std::vector< std::vector<double> >(this->num_patterns, std::vector<double>(this->num_states, 0.0) );
...
//                    sumPartialProbs += log( p_inv * ftotal * exp(this->perNodeSiteLogScalingFactors[this->active_branch_likelihood[node_index]][node_index][site]) + oneMinusPInv * per_mixture_Likelihoods[site] / this->num_site_rates ) * *patterns;
                }
                else
                {
                    sumPartialProbs += log( oneMinusPInv * per_mixture_Likelihoods[site] / this->num_site_rates ) * *patterns;
                }
//                sumPartialProbs -= this->perNodeSiteLogScalingFactors[this->active_branch_likelihood[node_index]][node_index][site] * *patterns;
...
void RevBayesCore::PhyloCTMCClado<charType>::updateTransitionProbabilities(size_t node_idx)
{
    ...
    if (useSampledCladogenesis)
    {
        ...
        else
        {
            ...
            TransitionProbabilityMatrix cp(this->num_states);
            for (size_t i = 0; i < this->num_states; i++)
                cp[i][i] = 0.0;
            cp[0][0] = 1.0;
            ...
            TransitionProbabilityMatrix tp(this->num_states);
            for (size_t i = 0; i < this->num_states; i++)
                tp[i][i] = 1.0;
...
void RevBayesCore::PhyloCTMCClado<charType>::redrawValue( void )
{
    ...
    for (size_t index = 0; index < this->changed_branches.size(); ++index)
    {
        if ( this->changed_branches[index] == false )
        {
            this->active_branch_likelihood[index] = (this->active_branch_likelihood[index] == 0 ? 1 : 0);
            this->changed_branches[index] = true;
        }
    }
