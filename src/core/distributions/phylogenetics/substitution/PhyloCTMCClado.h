...
void RevBayesCore::PhyloCTMCClado<charType>::computeRootLikelihood( size_t root, size_t left, size_t right)
{
    ...
    double* p_node         = this->getCreatePartialLikelihoodsForNode(root, this->num_site_mixtures, this->pattern_block_size, this->num_chars).likelihoods.data();
...
void RevBayesCore::PhyloCTMCClado<charType>::computeMarginalNodeLikelihood( size_t node_index, size_t parentnode_index )
{
    ...
    const double*   p_parent_node_marginal          = this->getPartialLikelihoodsForNode(parentnode_index).likelihoods.data();
