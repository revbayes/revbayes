...
PhyloBrownianRegressionProcess::PhyloBrownianRegressionProcess(const TypedDagNode<Tree> *t, const ContinuousCharacterData& pred) :
    AbstractPhyloBrownianProcess( t, 1 ),
 ...
 */
PhyloBrownianRegressionProcess::~PhyloBrownianRegressionProcess( void )
{
...
PhyloBrownianRegressionProcess* PhyloBrownianRegressionProcess::clone( void ) const
{
    
    return new PhyloBrownianRegressionProcess( *this );
}


double PhyloBrownianRegressionProcess::computeLnProbability( void )
{
...
void PhyloBrownianRegressionProcess::fireTreeChangeEvent( const TopologyNode &n, const unsigned& m )
{
...
double PhyloBrownianRegressionProcess::getSlope(size_t i) const
{
    double sl = 0;
    
    if ( single_slope != NULL )
    {
        sl = single_slope->getValue();
    }
    else if ( multiple_slope != NULL )
    {
        sl = multiple_slope->getValue()[i];
    }
    else
    {
        throw RbException("Neither single slope variable nor a vector was set.");
    }
...
double PhyloBrownianRegressionProcess::getMeanPredictor(size_t i) const
{
    
    double m = 0;
    
    if ( single_mean_predictor != NULL )
    {
        m = single_mean_predictor->getValue();
    }
    else if ( multiple_mean_predictor != NULL )
    {
        m = multiple_mean_predictor->getValue()[i];
    }
    else
    {
        throw RbException("Neither single mean variable nor a vector was set.");
    }
...
double PhyloBrownianRegressionProcess::getPredictor(size_t n, size_t site) const
{
    
    std::vector<TopologyNode*> nodes = this->tau->getValue().getNodes();
    
    ContinuousTaxonData& taxon = this->value->getTaxonData( nodes[n]->getName() );
    
    double c = taxon.getCharacter(site);
...
void PhyloBrownianRegressionProcess::keepSpecialization( const DagNode* affecter )
{
...
void PhyloBrownianRegressionProcess::recursiveComputeLnProbability( const TopologyNode &node, size_t node_index )
{
        ...
        for (size_t j = 1; j < num_children; ++j)
        {
            ...
            for (int site=0; site<this->num_sites; ++site)
            {
                ...
                else if ( use_missing_data == true && missing_data[left_index][site] == true && missing_data[right_index][site] == false )
                {
                    ...
                    if ( right.isTip() == true )
                    {
                        for ( size_t index_pred=0; index_pred<num_predictors; ++index_pred )
                        {
                            offset += getSlope( index_pred ) * ( getPredictor(right_index,index_pred) - getMeanPredictor(index_pred) );
                        }
                    }
                    mu_node[site] = mu_right[site] - offset;
                ...
                else if ( use_missing_data == true && missing_data[left_index][site] == false && missing_data[right_index][site] == true )
                {
                    ...
                    if ( left->isTip() == true )
                    {
                        for ( size_t index_pred=0; index_pred<num_predictors; ++index_pred )
                        {
                            offset += getSlope( index_pred ) * ( getPredictor(left_index,index_pred) - getMeanPredictor(index_pred) );
                        }
                    }
                    mu_node[site] = mu_left[site] - offset;
                ...
                else
                {
                    ...
                    if ( left->isTip() == true )
                    {
                        for ( size_t index_pred=0; index_pred<num_predictors; ++index_pred )
                        {
                            offset_left += getSlope( index_pred ) * ( getPredictor(left_index,index_pred) - getMeanPredictor(index_pred) );
                        }
                    }
                    ...
                    if ( right.isTip() == true )
                    {
                        for ( size_t index_pred=0; index_pred<num_predictors; ++index_pred )
                        {
                            offset_right += getSlope( index_pred ) * ( getPredictor(right_index,index_pred) - getMeanPredictor(index_pred) );
                        }
                    }

                    // compute the contrasts for this site and node
                    double contrast = (mu_left[site]-offset_left) - (mu_right[site]-offset_right);
                    ...
                    mu_node[site] = ((mu_left[site]-offset_left)*t_right + (mu_right[site]-offset_right)*t_left) / (t_left+t_right);
...
void PhyloBrownianRegressionProcess::recursivelyFlagNodeDirty( const TopologyNode &n )
{
...
void PhyloBrownianRegressionProcess::resetValue( void )
{
...
void PhyloBrownianRegressionProcess::restoreSpecialization( const DagNode* affecter )
{
...
void PhyloBrownianRegressionProcess::setMeanPredictor(const TypedDagNode< double >* m)
{
        
    // remove the old parameter first
    this->removeParameter( single_mean_predictor );
    this->removeParameter( multiple_mean_predictor );
    
    single_mean_predictor   = NULL;
    multiple_mean_predictor = NULL;
        
    // set the value
    single_mean_predictor = m;
    
    // add the new parameter
    this->addParameter( single_mean_predictor );
...
void PhyloBrownianRegressionProcess::setMeanPredictor(const TypedDagNode< RbVector< double > >* m)
{
    // remove the old parameter first
    this->removeParameter( single_mean_predictor );
    this->removeParameter( multiple_mean_predictor );
    ...
    // set the value
    multiple_mean_predictor = m;

    // add the new parameter
    this->addParameter( multiple_mean_predictor );
...
void PhyloBrownianRegressionProcess::setSlope(const TypedDagNode< double >* s)
{
    // remove the old parameter first
    this->removeParameter( single_slope );
    this->removeParameter( multiple_slope );
    ...
    // add the new parameter
    this->addParameter( single_slope );
...
void PhyloBrownianRegressionProcess::setSlope(const TypedDagNode< RbVector< double > >* s)
{
    // remove the old parameter first
    this->removeParameter( single_slope );
    this->removeParameter( multiple_slope );
    ...
    // add the new parameter
    this->addParameter( multiple_slope );
...
std::vector<double> PhyloBrownianRegressionProcess::simulateRootCharacters(size_t n)
{
...
double PhyloBrownianRegressionProcess::sumRootLikelihood( void )
{
    // get the root node
    const TopologyNode &root = this->tau->getValue().getRoot();
...
void PhyloBrownianRegressionProcess::touchSpecialization( const DagNode* affecter, bool touchAll )
{
...
void PhyloBrownianRegressionProcess::swapParameterInternal(const DagNode *oldP, const DagNode *newP)
{
