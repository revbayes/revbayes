...
//    // we need the counts for faster computation
    ...
    for (it_h = history.rbegin(); it_h != history.rend(); ++it_h)
    {
        ...
        double tr = rm.getRate(static_cast<CharacterEventDiscrete*>(curr_state[idx])->getState(), s, current_age, branch_rate);
...
/*
 * Mark the site-IID character-history caches dirty after a dependency changes.
 * Other dependencies use the base character-history invalidation logic.
 */
template<class charType>
void RevBayesCore::GeneralTreeHistoryCtmcSiteIID<charType>::invalidateSpecialization( const DagNode* affecter, bool fullyInvalidateSelf )
{
