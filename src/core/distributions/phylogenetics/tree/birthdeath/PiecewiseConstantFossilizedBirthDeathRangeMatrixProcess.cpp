 ...
 * \param[in]    tn             Taxa.
 */
PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess(const DagNode *inspeciation,
 ...
 */
PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess* PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::clone( void ) const
{
    return new PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess( *this );
}
 ...
 *
 */
double PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::computeLnProbability( void )
{
 ...
 * \return Small gamma
 */
void PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::updateGamma(bool force)
{
    for (size_t i = 0; i < fbd_taxa.size(); i++)
    {
 ...
 */
double PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::pSurvival(double start, double end) const
{
    double t = start;
 ...
 *
 */
void PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::updateStartEndTimes( void ) const
{
    origin = 0;
 ...
 */
void PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::redrawValue(void)
{
...
void PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::keepSpecialization(const DagNode *toucher)
{
    dirty_gamma = std::vector<bool>(fbd_taxa.size(), false);
}


void PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::restoreSpecialization(const DagNode *toucher)
{

}


void PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::touchSpecialization(const DagNode *toucher, bool touchAll)
{
    if ( toucher == dag_node )
    {
 ...
 */
void PiecewiseConstantFossilizedBirthDeathRangeMatrixProcess::swapParameterInternal(const DagNode *oldP, const DagNode *newP)
{
