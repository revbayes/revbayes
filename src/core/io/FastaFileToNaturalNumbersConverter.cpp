...
size_t FastaFileToNaturalNumbersConverter::getState(const std::vector<int>& counts, size_t n_alleles, size_t n_individuals, const std::vector<std::string>& vector_edges, const std::vector<int>& matrix_edges, size_t site, const std::string& taxa_name )
{
    ...
    if (n_counts==0)
    {
        throw RbException() << "Unexpected allelic counts at site " << site << " and taxa " << taxa_name << ". PoMos require at least one postive count.";
    }
    if (n_counts>2)
    {
        throw RbException() << "Unexpected allelic couts at site " << site << " and taxa " << taxa_name << ". PoMos only accept monoallelic or biallelic counts.";
    }
