...
size_t FastaFileToNaturalNumbersConverter::getIndex(const std::string& taxa_name, const std::vector<std::string>& taxa)
{
...
size_t FastaFileToNaturalNumbersConverter::getState(const std::vector<int>& counts, size_t n_alleles, size_t n_individuals, const std::vector<std::string>& vector_edges, const std::vector<int>& matrix_edges, size_t site, const std::string& taxa_name )
{
    ...
    if (n_counts==0)
    {
        throw RbException( "Unexpected allelic counts at site " + std::to_string(site) + " and taxa " + taxa_name + ". PoMos require at least one postive count.");
    }
    if (n_counts>2)
    {
        throw RbException( "Unexpected allelic couts at site " + std::to_string(site) + " and taxa " + taxa_name + ". PoMos only accept monoallelic or biallelic counts.");
    }
...
size_t FastaFileToNaturalNumbersConverter::sampleWeight(size_t M, size_t m, size_t N)
{
...
size_t FastaFileToNaturalNumbersConverter::sampleEdge(size_t allele_index, const std::vector<int>& vector)
{
...
size_t FastaFileToNaturalNumbersConverter::getIndex(const std::vector<std::string>& vector, const std::string& element)
{
