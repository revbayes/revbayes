...
int sum_submodels( const vector<unique_ptr<SiteMixtureModel>>& ms)
{
    int total = 0;
    for(auto& m: ms)
...
vector<unique_ptr<SiteMixtureModel>> clone_models(const vector<unique_ptr<SiteMixtureModel>>& ms)
{
    vector<unique_ptr<SiteMixtureModel>> submodels;
    for(auto& m: ms)
        submodels.push_back( unique_ptr<SiteMixtureModel>(m->clone()) );
    return submodels;
}
...
std::pair<int,int> get_indices(const vector<unique_ptr<SiteMixtureModel>>& ms, int i)
{
    int component = 0;
    for(auto& m: ms)
    {
...
ConcreteMixtureModel::ConcreteMixtureModel(const vector<unique_ptr<SiteMixtureModel>>& ms,
                                           const vector<double>& fs)
    :SiteMixtureModel( sum_submodels(ms), ms[0]->getNumberOfStates() ),
...
{
ConcreteMixtureModel* scaled_mixture(const SiteMixtureModel& sub_model, const std::vector<double>& fractions, const std::vector<double>& rates)
{
    vector<unique_ptr<SiteMixtureModel>> sub_models;
    for(auto& rate: rates)
    {
        auto sub_model2 = std::unique_ptr<SiteMixtureModel>(sub_model.clone());
