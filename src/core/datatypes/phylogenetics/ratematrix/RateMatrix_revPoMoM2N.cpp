...
void RateMatrix_revPoMoM2N::computeOffDiagonal( void )
{
    ...
    m[0][2]   = mu[0];    //mutation 01
    m[1][M]   = mu[1];    //mutation 10
    ...
    m[2][3]   = (M-1.0)*cons;  //{(N-1)a0,1a1} -> {(N-2)a0,2a1}
    m[2][0]   = (M-1.0)*cons;  //{(N-1)a0,1a1} -> {Na0,0a1}
    m[2][2]   = -m[2][3]-m[2][0];
    for (int v=2; v<(M-1); v++)
    {
        m[v+1][v+2] = 1.0*v*(M-v)*cons;   //{(N-v)a0,va1} -> {(N-v-1)a0,(v+1)a1}
        m[v+1][v]   = 1.0*v*(M-v)*cons;   //{(N-v)a0,va1} -> {(N-v+1)a0,(v-1)a1}
        m[v+1][v+1] = -m[v+1][v+2]-m[v+1][v];
    }
    m[M][1]   = (M-1.0)*cons;  //{1a0,(N-1)a1} -> {0a0,Na1}
    m[M][M-1] = (M-1.0)*cons;  //{1a0,(N-1)a1} -> {2a0,(N-2)a1}
    m[M][M]   = -m[M][1]-m[M][M-1];
