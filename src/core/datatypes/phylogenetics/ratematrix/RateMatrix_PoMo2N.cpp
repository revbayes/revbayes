...
RateMatrix_PoMo2N::RateMatrix_PoMo2N( long num_states, long in_n ) : TimeReversibleRateMatrix( num_states ),
    N( in_n ),
    mu( 2 , 0.01 ),
    phi( 2 , 1.0 )
{
...
/** Copy constructor */
RateMatrix_PoMo2N::RateMatrix_PoMo2N(const RateMatrix_PoMo2N& m) : TimeReversibleRateMatrix( m ),
    N( m.N ),
    mu( m.mu ),
    phi( m.phi )
{
...
void RateMatrix_PoMo2N::computeOffDiagonal( void )
{
...
//mutations
  m[0][2]  = mu[0];  //{Na0} -> {(N-1)a0,1a1}
  m[1][N]  = mu[1];  //{Na1} -> {1a0,(N-1)a1}
