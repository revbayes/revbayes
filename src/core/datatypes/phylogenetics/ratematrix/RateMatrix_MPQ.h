...
namespace RevBayesCore {
    ...
    public:
        ...
        long                                getEigenUpdateCount(void) const { return eigenUpdateCount; }                          //!< How many eigendecompositions have been done, for diagnostics
        static long                         getNumClampedTiProbs(void) { return numClampedTiProbs; }
        static double                       getWorstNegativeTiProb(void) { return worstNegativeTiProb; }
        static double                       getWorstRowSumError(void) { return worstRowSumError; }
    ...
    private:
        ...
        void                                updateIfNeeded(void) const;
#ifdef MPQ_VERIFY_TIPROBS
        void                                verifyTransitionProbabilities(double t, TransitionProbabilityMatrix& P) const;   //!< compare against exp(Qt); see the .cpp
#endif                                      //!< Recompute the eigensystem, but only if the matrix has changed since it was last computed
        
        void                                calculateCijk(void);                                                                //!< Do precalculations on eigenvectors and their inverse
        void                                tiProbsExpm(double t, TransitionProbabilityMatrix& P) const;                        //!< Transition probabilities by scaling and squaring; the default. See the .cpp
        ...
        mutable long                        eigenUpdateCount;

        static long                         numClampedTiProbs;
        static double                       worstNegativeTiProb;
        static double                       worstRowSumError;
