...
tree ~ dnCBDSP(rootAge    = root_age,
	...
	help_references[string("dnCBDSP")].push_back(RbHelpReference(R"("H\xF6hna S, Freyman WA, Nolen Z, Huelsenbeck JP, May MR, Moore BR (2019).  A Bayesian approach for estimating branch-specific speciation and extinction  rates. bioRxiv.")",R"(10.1101/555805)",R"(https://www.biorxiv.org/content/10.1101/555805v1.full )"));
	help_references[string("dnCBDSP")].push_back(RbHelpReference(R"("Moore BR, H\xF6hna S, May MR, Rannala B, Huelsenbeck JP (2016). Critically  evaluating the theory and performance of Bayesian analysis of macroevolutionary  mixtures. Proceedings of the National Academy of Sciences of the USA, 113(34):9569-9574.")",R"(10.1073/pnas.1518659113)",R"(https://www.pnas.org/doi/full/10.1073/pnas.1518659113 )"));
...
timetree ~ dnCDBDP(rootAge   = root_age,
	...
	help_references[string("dnCDBDP")].push_back(RbHelpReference(R"("H\xF6hna S, Freyman WA, Nolen Z, Huelsenbeck JP, May MR, Moore BR (2019).  A Bayesian approach for estimating branch-specific speciation and extinction  rates. bioRxiv.")",R"(10.1101/555805)",R"(https://www.biorxiv.org/content/10.1101/555805v1.full )"));
...
# continue as usual to either clamp the genealogy or infer the genealogy based on sequence data)");
	...
	help_references[string("dnCoalescent")].push_back(RbHelpReference(R"("Billenstein R, H\xF6hna S (2024). Comparison of Bayesian coalescent skyline  plot models for inferring demographic histories. Molecular Biology and Evolution,  41(5):msae073.")",R"(10.1093/molbev/msae073)",R"(https://academic.oup.com/mbe/article/41/5/msae073/7648822 )"));
...
# continue as usual to either clamp the genealogy or infer the genealogy based on sequence data)");
	...
	help_references[string("dnCoalescentSkyline")].push_back(RbHelpReference(R"("Billenstein R, H\xF6hna S (2024). Comparison of Bayesian coalescent skyline  plot models for inferring demographic histories. Molecular Biology and Evolution,  41(5):msae073.")",R"(10.1093/molbev/msae073)",R"(https://academic.oup.com/mbe/article/41/5/msae073/7648822 )"));
...
writeNexus("simulatedData.nex", sim))");
	...
	help_references[string("dnPhyloCTMC")].push_back(RbHelpReference(R"("H\xF6hna S, Landis MJ, Heath TA (2017). Phylogenetic inference using  `RevBayes`. Current Protocols in Bioinformatics, 57:6.16.1--6.16.34.")",R"(10.1002/cpbi.22)",R"()"));
...
mymcmcObject.run( rules = [ srMaxIteration(400) ] ))");
	...
	help_references[string("mcmc")].push_back(RbHelpReference(R"("H\xF6hna S, Landis MJ, Heath TA (2017). Phylogenetic inference using  `RevBayes`. Current Protocols in Bioinformatics, 57:6.16.1--6.16.34.")",R"(10.1002/cpbi.22)",R"()"));
...
moves.append( mvFNPR(tree, weight=taxa.size()) ))");
	...
	help_references[string("mvFNPR")].push_back(RbHelpReference(R"("H\xF6hna S, Defoin-Platel M, Drummond AJ (2008). Clock-constrained tree  proposal operators in Bayesian phylogenetic inference. 1--7 in 8th IEEE International  Conference on BioInformatics and BioEngineering (BIBE 2008). Athens, Greece,  October 2008.")",R"(10.1109/BIBE.2008.4696663)",R"(https://alexeidrummond.org/assets/publications/2008-hoehna-clock-bibe.pdf )"));
...
mymcmc = mcmc(mymodel, monitors, moves)
mymcmc.run(generations=200000))");
	help_strings[string("mvIidPrior")][string("name")] = string(R"(mvIidPrior)");
...
mymcmc.burnin(generations=20000,tuningInterval=100)
mymcmc.run(generations=200000))");
	help_strings[string("mvRandomNaturalWalk")][string("name")] = string(R"(mvRandomNaturalWalk)");
...
moves.append( mvScaleBactrian(speciation_rate, weight=5) ))");
	...
	help_references[string("mvScaleBactrian")].push_back(RbHelpReference(R"("Yang Z, Rodr\xEDguez CE (2013). Searching for efficient Markov chain  Monte Carlo proposal kernels. Proc. Natl. Acad. Sci. USA, 110(48):19307-19312.")",R"(10.1073/pnas.1311790110)",R"(https://www.pnas.org/doi/full/10.1073/pnas.1311790110 )"));
...
moves.append( mvSlideBactrian(x, tune=TRUE, weight=1) ))");
	...
	help_references[string("mvSlideBactrian")].push_back(RbHelpReference(R"("Yang Z, Rodr\xEDguez CE (2013). Searching for efficient Markov chain  Monte Carlo proposal kernels. Proc. Natl. Acad. Sci. USA, 110(48):19307-19312.")",R"(10.1073/pnas.1311790110)",R"(https://www.pnas.org/doi/full/10.1073/pnas.1311790110 )"));
...
ps.stdError(bootstrap=TRUE, replicates=50, printFiles=TRUE) # bootstrap (50 replicates, save output))");
	...
	help_references[string("pathSampler")].push_back(RbHelpReference(R"("\xC1lvarez-Carretero S, Tamuri AU, Battini M, Nascimento FF, Carlisle  E, Asher RJ, Yang Z, Donoghue PCJ, dos Reis M (2022). A species-level timeline  of mammal evolution integrating phylogenomic data. Nature, 602(7896):263-267.")",R"(10.1038/s41586-021-04341-1)",R"(https://www.nature.com/articles/s41586-021-04341-1 )"));
...
mymcmc.run(rules = stopping_rules))");
	...
	help_references[string("srMinESS")].push_back(RbHelpReference(R"("Guimar\xE3es Fabreti L, H\xF6hna S (2022). Convergence assessment for  Bayesian phylogenetic analysis using MCMC simulation. Methods in Ecology and  Evolution, 13(1):77--90.")",R"(10.1111/2041-210X.13727)",R"(https://besjournals.onlinelibrary.wiley.com/doi/10.1111/2041-210X.13727 )"));
...
ss.stdError(bootstrap=TRUE, replicates=50, printFiles=TRUE) # bootstrap (50 replicates, save output))");
	...
	help_references[string("steppingStoneSampler")].push_back(RbHelpReference(R"("\xC1lvarez-Carretero S, Tamuri AU, Battini M, Nascimento FF, Carlisle  E, Asher RJ, Yang Z, Donoghue PCJ, dos Reis M (2022). A species-level timeline  of mammal evolution integrating phylogenomic data. Nature, 602(7896):263-267.")",R"(10.1038/s41586-021-04341-1)",R"(https://www.nature.com/articles/s41586-021-04341-1 )"));
