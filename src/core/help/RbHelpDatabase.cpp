...
mymcmc.run(generations=200000))");
	...
	help_arrays[string("dnPhyloBrownianStateDependent")][string("authors")].push_back(string(R"(Priscilla Lau)"));
	help_strings[string("dnPhyloBrownianStateDependent")][string("description")] = string(R"(Univariate Brownian process over an augmented phylogeny (i.e., discrete character history))");
	help_strings[string("dnPhyloBrownianStateDependent")][string("details")] = string(R"(The phylogenetic state-dependent Brownian process is collapsed from the phylogenetic state-dependent Ornstein-Uhlenbeck process, in which `alpha` is always 0.
The probability is computed using a pruing algorithm.
Specifically, `dnPhyloBMSD` uses an augmented tree structure, i.e., the character history with different character states (also referred to as "regimes"; Hansen 1997).
...
Under this process, branches of the same character state share the same OU parameters (sigma).
These character states can correspond to the states of an observed discrete character.

This model is equivalent to the MuSSCRat model (May and Moore 2020) if (1) the continuous trait is univariate, and (2) we assume no background rate variation.
The major difference between these two models lies in the probability computation.
The MuSSCRat model transforms the branch lengths according to the `sigma` parameter and computes the probability using a state-independent algorithm.
The phyloBMSD model does not transform the phylogeny and computes the probability using a state-dependent algorithm.
...
The `sigma` arugments represents the state-dependent diffusion parameter that controls continuous trait evolution in different discrete character states.
...
The `sigma` arugments represents the state-dependent diffusion parameter that controls continuous trait evolution in different discrete character states.
The continuous trait observations are fixed to the tips using `.clamp()`.)");
	help_strings[string("dnPhyloBrownianStateDependent")][string("example")] = string(R"(# setup for a two-state phyloOUSD model
num_states = 2          # 0 and 1 are the only states
...
Y ~ dnPhyloOUSD(char_hist, sigma=sigma2^0.5))");
	...
	help_arrays[string("dnPhyloBrownianStateDependent")][string("see_also")].push_back(string(R"(dnPhyloOUSD)"));
	help_arrays[string("dnPhyloBrownianStateDependent")][string("see_also")].push_back(string(R"(dnPhyloCTMCDASiteIID)"));
	help_arrays[string("dnPhyloBrownianStateDependent")][string("see_also")].push_back(string(R"(readCharacterHistory)"));
	help_strings[string("dnPhyloBrownianStateDependent")][string("title")] = string(R"(Phylogenetic state-dependent Brownian process)");
...
x)");
	...
	help_strings[string("dnPhyloOrnsteinUhlenbeckStateDependent")][string("description")] = string(R"(Univariate Ornstein-Uhlenbeck process over an augmented phylogeny (i.e., discrete character history))");
	help_strings[string("dnPhyloOrnsteinUhlenbeckStateDependent")][string("details")] = string(R"(The phylogenetic state-dependent Ornstein-Uhlenbeck process is an extension toto Hansen's (1997) Ornstein-Uhlenbeck process with state-dependent optima.
...
Y ~ dnPhyloOUSD(char_hist, theta=theta, rootTreatment="optimum", alpha=alpha, sigma=sigma2^0.5))");
	help_strings[string("dnPhyloOrnsteinUhlenbeckStateDependent")][string("name")] = string(R"(dnPhyloOrnsteinUhlenbeckStateDependent (dnPhyloOUSD))");
