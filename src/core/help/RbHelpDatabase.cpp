...
mymcmc.run(generations=200000))");
	...
	help_strings[string("dnBirthDeathSamplingTreatmentProcess")][string("name")] = string(R"(dnBirthDeathSamplingTreatment)");
...
moves.append( mvTipTimeSlideUniform(bd, weight = taxa.size()) ))");
	help_strings[string("dnFossilizedBirthDeathProcess")][string("name")] = string(R"(dnFossilizedBirthDeath)");
	help_references[string("dnFossilizedBirthDeathProcess")].push_back(RbHelpReference(R"(The fossilized birth-death model for the analysis of stratigraphic range data under different speciation modes. Stadler, Tanja et al. Journal of theoretical biology, 447:41-55.)",R"()",R"(https://www.sciencedirect.com/science/article/pii/S002251931830119X )"));
	help_arrays[string("dnFossilizedBirthDeathProcess")][string("see_also")].push_back(string(R"(dnFossilizedBirthDeathRange)"));
	help_arrays[string("dnFossilizedBirthDeathProcess")][string("see_also")].push_back(string(R"(dnBirthDeathSamplingTreatment)"));
	help_arrays[string("dnFossilizedBirthDeathRangeProcess")][string("authors")].push_back(string(R"(Walker Pett)"));
	help_strings[string("dnFossilizedBirthDeathRangeProcess")][string("description")] = string(R"(The fossilized birth death range process (FBDRP) describes the distribution of a matrix of species origination and extinction times under a model of asymmetric speciation and sampling of extinct species.)");
	help_strings[string("dnFossilizedBirthDeathRangeProcess")][string("details")] = string(R"(Fossil species are represented by a collection of fossil occurrences with uncertainty. Speciation, extinction and sampling rates may be time-homogeneous or piecewise time-heterogeneous. If time-heterogeneous rates are provided, then a vector of rate change time-points musts also be provided. If only a subset of fossil occurrences is provided for each species (e.g. only first and last occurrencces), then the remaining number of fossil samples may be marginalized by specifying `complete=FALSE`. Under the hood, the fossil data is augmented with oldest occurrence ages for each species, which are automatically marginalized during when the model is sampled using MCMC. To disable this behavior, use `resample=FALSE`.)");
	help_strings[string("dnFossilizedBirthDeathRangeProcess")][string("example")] = string(R"(lambda ~ dnExp(10)
mu ~ dnExp(10)
psi ~ dnExp(10)
...
moves.append( mvMatrixElementSlide(bd, weight=taxa.size()) ))");
	help_strings[string("dnFossilizedBirthDeathRangeProcess")][string("name")] = string(R"(dnFossilizedBirthDeathRange)");
	help_references[string("dnFossilizedBirthDeathRangeProcess")].push_back(RbHelpReference(R"(The fossilized birth-death model for the analysis of stratigraphic range data under different speciation modes. Stadler, Tanja et al. Journal of theoretical biology, 447:41-55.)",R"()",R"(https://www.sciencedirect.com/science/article/pii/S002251931830119X )"));
	help_arrays[string("dnFossilizedBirthDeathRangeProcess")][string("see_also")].push_back(string(R"(dnFossilizedBirthDeath)"));
	help_arrays[string("dnFossilizedBirthDeathRangeProcess")][string("see_also")].push_back(string(R"(dnBirthDeathSamplingTreatment)"));
...
Variables on [0,infinity) are log-transformed for proposals.)");
	...
	help_strings[string("mvResampleFBD")][string("description")] = string(R"(This move resamples an oldest occurrence age for a random species in a fossilized birth death process described by `dnFBDP` or `dnFBDRP`)");
...
moves.append( mvResampleFBD(bd, weight=taxa.size()) ))");
	...
	help_arrays[string("mvResampleFBD")][string("see_also")].push_back(string(R"(dnFossilizedBirthDeath)"));
	help_arrays[string("mvResampleFBD")][string("see_also")].push_back(string(R"(dnBirthDeathBirthDeathRange)"));
