...
map_tree = consensusTree(trace=tree_trace, cutoff=0.5, file="consensus.tree"))");
	...
	help_arrays[string("dnBDS")][string("authors")].push_back(string(R"(June Walker)"));
...
moves.append( mvMatrixElementSlide(bd, weight=taxa.size()) ))");
	help_strings[string("dnBDS")][string("name")] = string(R"(dnBDS)");
	...
	help_strings[string("dnBDS")][string("title")] = string(R"(The birth-death-with-rateshifts range process)");
...
x.probability())");
	...
	help_arrays[string("dnFossilRecord")][string("authors")].push_back(string(R"(June Walker)"));
	help_strings[string("dnFossilRecord")][string("description")] = string(R"(The fossil record of a set of species, conditioned on a fossilized birth-death range process: the probability of the observed fossil occurrences given the species ranges that produced them.)");
	help_strings[string("dnFossilRecord")][string("details")] = string(R"(This is the observation half of the fossilized birth-death range model. The range process (`dnFBDRP` or `dnFBDSP`) describes how species ranges diversify; this distribution gives the probability of the fossil record itself.
...
moves.append( mvMatrixElementScale(bd, weight=taxa.size()) )
moves.append( mvMatrixElementSlide(bd, weight=taxa.size()) ))");
	help_strings[string("dnFossilRecord")][string("name")] = string(R"(dnFossilRecord)");
	...
	help_strings[string("dnFossilRecord")][string("title")] = string(R"(The fossil record of a set of stratigraphic ranges)");
...
moves.append( mvMatrixElementSlide(bd, weight=taxa.size()) ))");
	...
	help_strings[string("dnFossilizedBirthDeathRange")][string("title")] = string(R"(The fossilized birth-death range process)");
...
moves.append( mvStratigraphicRange(bd, weight = taxa.size()) ))");
	...
	help_strings[string("dnFossilizedBirthDeathSpeciation")][string("title")] = string(R"(The fossilized birth-death range process over trees)");
...
mv[6] = mvVectorSlide(x))");
	...
	help_arrays[string("dnNbinomial")][string("authors")].push_back(string(R"(June Walker)"));
...
Y ~ dnPhyloOUSD(char_hist, theta=theta, rootTreatment="optimum", alpha=alpha, sigma=sigma2^0.5))");
	...
	help_arrays[string("dnPointMass")][string("authors")].push_back(string(R"(June Walker)"));
...
moves.append( mvCharacterHistory(ctmc=X, qmap_site=Q, graph="tip",
	...
	help_strings[string("mvCollapseExpandFossilBranch")][string("title")] = string(R"(Collapse a fossil tip onto its parent, or expand it back)");
...
mymcmc.operatorSummary())");
	...
	help_strings[string("mvMatrixElementScale")][string("details")] = string(R"(`row` and `col` confine the move to one line of the matrix, counting from 1; 0, the default, draws from every element. Giving both pins a single element. This is for a matrix whose margins hold different quantities: the birth-death matrix of `dnFBDRP` has one row per taxon and holds births in column 1 and deaths in column 2, so `col=1` scaling only the birth times.)");
	help_strings[string("mvMatrixElementScale")][string("name")] = string(R"(mvMatrixElementScale)");
	help_strings[string("mvMatrixElementScale")][string("title")] = string(R"(Scale one element of a matrix)");
	help_strings[string("mvMatrixElementSlide")][string("details")] = string(R"(`row` and `col` confine the move to one line of the matrix, counting from 1; 0, the default, draws from every element. Giving both pins a single element. This is for a matrix whose margins hold different quantities: the birth-death matrix of `dnFBDRP` has one row per taxon and holds births in column 1 and deaths in column 2, so `col=1` sliding only the birth times.)");
	help_strings[string("mvMatrixElementSlide")][string("name")] = string(R"(mvMatrixElementSlide)");
	help_strings[string("mvMatrixElementSlide")][string("title")] = string(R"(Slide one element of a matrix)");
